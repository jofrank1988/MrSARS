...
#Fetches the scores from the reference species, and then aggregates them
def normagg():
    global main_m
    global ordered_refs_list
    ...
    for elt in obs_list:
        if elt.is_ref == 1:
            ordered_refs_list.append(elt.name)
            if temp == None:
                temp = Counter(elt.scores)
            else:
                temp += Counter(elt.scores)
            for key in elt.scores:
                main_m.update({key:main_m[key]+[elt.scores[key]]})
    for key in temp:
...
def output(cli = 0):
    ...
    df.insert(0,"Species",ordered_refs_list + ["Aggregate"])
    df = df.set_index("Species")
    df = df.transpose()
    #print(df)
    if cli == 1:
        df.to_csv(similarity_file, index=True)
    else:
        out = df.to_csv(index=True)
